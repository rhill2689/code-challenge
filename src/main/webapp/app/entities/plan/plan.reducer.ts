...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_PLAN: 'plan/PARTIAL_UPDATE_PLAN',
...
export default (state: PlanState = initialState, action): PlanState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_PLAN):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_PLAN):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_PLAN):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<IPlan> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_PLAN,
